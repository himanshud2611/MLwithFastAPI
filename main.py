...
#fastapi works with type hinting
#ORSMiddleware allows you to configure which origins are allowed to make requests to your API.
from fastapi.middleware.cors import CORSMiddleware
...
@app.post("/predict-mage/")
async def predict_image(file: UploadFile = File(...)):
    contents = await file.read()
    pil_image = PIL.Image.open(io.BytesIO(contents)).convert('L') #open bytestream of contents and convert it to grayscale

    pil_iamge = PIL.ImageOps.invert(pil_iamge)
    pil_iamge = pil_iamge.resize((28,28), PIL.Image.ANTIALIAS)
